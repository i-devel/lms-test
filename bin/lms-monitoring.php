...
// kasowanie starych wpisów, żeby nie męczyć za każdym razem bazy 
// kasowanie jest w godzinach od 02:00 do 03:00

$data = strtotime(date('Y/m/d',time()));

if ( (time() >= ($data + 7200)) && (time() <= ($data + 10800)) )
{
    // czyszczenie statów dla kompów
    if (get_conf('monit.nodes_clear') !='' && get_conf('monit.nodes_clear') != '0' && (get_conf('monit.nodes_clear',0) > '7')) 
    {
	$leftday = $data - (intval(get_conf('monit.nodes_clear')) * 86400);
	if ($ideki = $DB->GetCol('SELECT v.id AS id FROM monit_vnodes v JOIN monittime t ON (t.nodeid = v.id) WHERE v.netdev = 0 AND t.cdate < ?',array($leftday)))
	{
    ...
    // czyszczenie dla urz. sieciowych
    if (get_conf('monit.netdev_clear') !='' && get_conf('monit.netdev_clear') != '0' && (get_conf('monit.netdev_clear',0) > '7')) 
    {
	$leftday = $data - (intval(get_conf('monit.nodes_clear')) * 86400);
	if ($ideki = $DB->GetCol('SELECT v.id AS id FROM monit_vnodes v JOIN monittime t ON (t.nodeid = v.id) WHERE v.netdev = 1 AND t.cdate < ?',array($leftday)))
	{
	    $DB->Execute('DELETE FROM monitwarn WHERE nodeid IN ('.implode(',',$ideki).')');
	    $DB->Execute('DELETE FROM monittime WHERE nodeid IN ('.implode(',',$ideki).')');
	}
    ...
    // czyszczenie dla własnych urządzeń
    if (get_conf('monit.owner_clear') !='' && get_conf('monit.owner_clear') != '0' && (get_conf('monit.owner_clear',0) > '7')) 
    {
	$leftday = $data - (intval(get_conf('monit.nodes_clear')) * 86400);
